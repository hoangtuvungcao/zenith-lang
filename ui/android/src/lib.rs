// Android-specific Zenith UI implementation
// Uses Android NDK + eframe with android-game-activity
...
fn create_mock_ui(count: i64) -> Value {
    ...
    heading.insert(
        "text".to_string(),
        Value::String("🤖 Zenith Android Demo".to_string()),
    );
    ...
    text.insert(
        "text".to_string(),
        Value::String(format!("Current Count: {}", count)),
    );
    text.insert("size".to_string(), Value::Integer(24));
    text.insert("bold".to_string(), Value::Boolean(true));
    text.insert("color".to_string(), Value::String("green".to_string()));
    ...
    row.insert(
        "children".to_string(),
        Value::Array(Arc::new(Mutex::new(row_children))),
    );
    children.push(Value::Object(Arc::new(Mutex::new(row))));

    col.insert(
        "children".to_string(),
        Value::Array(Arc::new(Mutex::new(children))),
    );
    card.insert(
        "child".to_string(),
        Value::Object(Arc::new(Mutex::new(col))),
    );
    root.insert(
        "child".to_string(),
        Value::Object(Arc::new(Mutex::new(card))),
    );

    Value::Object(Arc::new(Mutex::new(root)))
}

impl eframe::App for AndroidRenderer {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let event_handler: EventHandler =
            Box::new(move |action: String, payload: Option<String>| {
...
fn android_main(app: AndroidApp) {
    ...
    eframe::run_native(
        "Zenith",
        options,
        Box::new(|_cc| Box::new(AndroidRenderer::new())),
    )
